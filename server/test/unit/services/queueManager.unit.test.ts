  ...
  describe('enqueue', () => {
    ...
    describe('Service Validation Errors', () => {
      ...
      it('should throw NotFoundError when service does not exist', async () => {
        ...
        // Act & Assert
        await expect(queueManager.enqueue(999))
          .rejects
        ...
        expect(mockServiceFindUnique).toHaveBeenCalledWith({ where: { id: 999 } });
      ...
      it('should handle service lookup database errors', async () => {
        // Arrange
        mockServiceFindUnique.mockRejectedValue(new Error('Database connection failed'));

        // Act & Assert
        await expect(queueManager.enqueue(mockServiceId))
          .rejects
    ...
    describe('Queue Loading Errors', () => {
      ...
      it('should throw InternalServerError when loading waiting tickets fails', async () => {
        ...
        mockServiceFindUnique.mockResolvedValue(mockService);
        mockTicketFindMany.mockRejectedValue(new Error('Failed to fetch waiting tickets'));
    ...
    describe('Edge Cases', () => {
      ...
      it('should handle very large queue sizes', async () => {
        ...
        const largeQueue = Array.from({ length: 1000 }, (_, i) => ({ code: `D-${String(i + 1).padStart(3, '0')}` }));
        mockServiceFindUnique.mockResolvedValue(mockService);
